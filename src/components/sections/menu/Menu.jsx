  ...
  return (
    <section ref={containerRef} id="menu" className="py-20 px-4 md:px-8 bg:[#F8F8F8] overflow-hidden">
      ...
      <span className="inline-block w-60 -mt-10 h-1 bg-[#FFDAC9]" style={{ marginLeft: "33rem"}} />
        <h2 className="text-4xl font-bold text-center mb-12" style={{ marginTop: "-1.5rem", marginLeft: "0.7rem"}}>OUR MENU</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          {Object.entries(menuData).map(([category, items], index) => (
            <motion.div
              ...
              className="space-y-8"
            >
              <h3 className={`text-2xl font-bold mb-6 ${category === "MAIN COURSES" ? "-ml-3" : category === "DESSERTS" ? " -ml-6" : "-ml-4"}  -z-10 overflow-wrap: break-word`} style={{ width: ".2rem"}}>{category}</h3>
